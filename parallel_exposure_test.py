#!/usr/bin/env python3
"""
Parallel comprehensive test script that coordinates both accounts to:
...
# Account credentials with both MM and web authentication
ACCOUNTS = [
    {
        "id": 1,
...
shared_data = {
    "tournament_id": None,
    "event_ids": [],
    "market_data": [],  # List of {event_id, market_id, line_ids}
    "lock": threading.Lock()
}

class BetProphetTester:
    ...
    def get_events_and_markets(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Store event IDs
                event_ids = [event['event_id'] for event in events[:3]]  # Use first 3 events
                with shared_data["lock"]:
...
# Get markets for these events
                                        ...
                                        if isinstance(selection_group, list):
                                            for selection in selection_group:
                                                if selection.get('line_id'):
                                                    line_ids.append(selection['line_id'])
                                        elif isinstance(selection_group, dict) and selection_group.get('line_id'):
                                            line_ids.append(selection_group['line_id'])
                                
                                # Also check if there are direct line_ids in the market
                                if not line_ids and market.get('line_ids'):
                                    line_ids = market['line_ids']
                                
                                if line_ids:
                                    # Store selection details for better understanding
                                    selection_details = []
                                    if selections:
                                        for selection_group in selections:
                                            if isinstance(selection_group, list):
                                    ...
                                    print(f"   ✅ Market added - Event: {event_id}, Market: {market_id}, Type: {market_type}, Lines: {len(line_ids)}")
                                    # Print selection details for debugging
                                    print(f"   📋 MARKET DEBUG - Total selections found: {len(selection_details)}")
                                    for i, detail in enumerate(selection_details):  # Show all selections
                                        print(f"      Selection {i+1}: {detail['name']} (ID: {detail['line_id'][:8]}..., Odds: {detail['odds']}, Outcome: {detail['outcome_id']})")
                                    
                                    # Also show raw market selections for deeper debugging
                                    print(f"   🔍 RAW SELECTIONS DEBUG: {json.dumps(selections[:2], indent=4)}")
                                else:
    ...
    def monitor_exposure_changes(self, initial_gec, initial_lec_data, event_ids, market_ids):
        """Monitor for exposure changes after betting"""
        print(f"👀 {self.account['name']} - Starting exposure monitoring...")
        
        for i in range(10):  # Monitor for up to 10 iterations
            time.sleep(3)  # Wait 3 seconds between checks
            
            # Check wallet for GEC changes
            ...
            if current_wallet:
                ...
                if gec_change != 0:
                    print(f"🚨 {self.account['name']} - GEC CHANGED!")
                    print(f"   Initial GEC: ${initial_gec}")
                    print(f"   Current GEC: ${current_gec}")
                    print(f"   Change: ${gec_change}")
            
            # Check LEC changes
            current_lec = self.check_exposures(event_ids, market_ids)
            if current_lec and current_lec.get('data'):
                if len(current_lec['data']) > 0:
                    print(f"🚨 {self.account['name']} - LEC DATA FOUND!")
                    print(f"   LEC Data: {json.dumps(current_lec['data'], indent=2)}")
            
            print(f"⏱️ {self.account['name']} - Monitoring iteration {i+1}/10")
...
def coordinate_betting_for_gec_generation(authenticated_testers):
    """Coordinate betting between both accounts to generate GEC from 3 events"""
    print("\n" + "="*80)
    print("🎯 COORDINATED BETTING FOR GEC GENERATION")
    print("="*80)
    print("🎯 Target: Generate GEC from 3 different events")
    ...
    events_with_gec = set()  # Track which events have generated GEC
    target_gec_events = 3    # Target number of events to generate GEC from
    ...
    # Process each event until we have GEC from 3 events
    for event_id in events_markets.keys():
        if len(events_with_gec) >= target_gec_events:
            print(f"🎉 Target achieved! Generated GEC from {len(events_with_gec)} events")
            break
        
        print(f"\n" + "="*60)
        print(f"🎯 PROCESSING EVENT {event_id}")
        print(f"🎯 Events with GEC so far: {len(events_with_gec)}/{target_gec_events}")
        print("="*60)
        ...
        bet_round = 1
        max_bet_rounds = 20  # Prevent infinite loop
        gec_generated = False
        
        while not gec_generated and bet_round <= max_bet_rounds:
            print(f"\n🎲 BET ROUND {bet_round} for Event {event_id}")
            
            # Select market for this round (cycle through available markets)
            ...
            if len(target_market['line_ids']) < 2:
                print(f"   ⚠️ Skipping market with insufficient lines")
                bet_round += 1
                continue
            
            # Sequential alternating betting pattern - accounts take turns being maker/taker
            stake = 5.0 + (bet_round * 2.0)  # Increase stake each round
            
            # Ensure we have at least 2 teams for opposing bets
            if len(target_market['selections']) < 2:
                print(f"   ⚠️ Need at least 2 teams for opposing bets: {len(target_market['selections'])}")
                bet_round += 1
                continue
            ...
            if bet_round % 2 == 1:  # Odd rounds: Account 1 leads on Team A, Account 2 follows on Team B
                maker_account = testers[0]  # Account 1 (usr002)
                taker_account = testers[1]  # Account 2 (usr001)
                maker_team = team_outcome_4  # Team A
                taker_team = team_outcome_5  # Team B (opposing)
                maker_odds = 160
                taker_odds = -160
                print(f"   🏆 Round {bet_round}: Account 1 leads on {maker_team['name']} (outcome {maker_team['outcome_id']}), Account 2 follows on {taker_team['name']} (outcome {taker_team['outcome_id']})")
            else:  # Even rounds: Account 2 leads on Team B, Account 1 follows on Team A  
                maker_account = testers[1]  # Account 2 (usr001)
                taker_account = testers[0]  # Account 1 (usr002)
                maker_team = team_outcome_5  # Team B
                taker_team = team_outcome_4  # Team A (opposing)
                maker_odds = 160
                taker_odds = -160
                print(f"   🏆 Round {bet_round}: Account 2 leads on {maker_team['name']} (outcome {maker_team['outcome_id']}), Account 1 follows on {taker_team['name']} (outcome {taker_team['outcome_id']})")
            
            # Step 1: Maker places initial bet (creates open position)
            ...
            # Wait a moment for the position to be available
            print(f"   ⏱️ Waiting 3 seconds for maker bet to be available for matching...")
            time.sleep(3)
            
            # Step 2: Taker matches the bet (bets on opposing team)
            print(f"   🎯 STEP 2: {taker_account.account['name']} (TAKER) matching by betting on {taker_team['name']}")
            ...
            successful_bets = []
            if maker_bet:
                successful_bets.append(maker_bet)
            if taker_bet:
                successful_bets.append(taker_bet)
            
            bet_results = successful_bets
            
            successful_bets = [result for result in bet_results if result is not None]
            print(f"   📊 Round {bet_round}: {len(successful_bets)}/2 bets successful")
    ...
    print(f"="*80)
    print(f"🎯 Target: Generate GEC from {target_gec_events} events")
    ...
    if len(events_with_gec) >= target_gec_events:
        ...
        print(f"⚠️ Partial success: Generated GEC from {len(events_with_gec)}/{target_gec_events} events")
        return len(events_with_gec) > 0

def main():
    """Main function to run parallel comprehensive testing"""
    print("="*80)
    print("🚀 PARALLEL EXPOSURE TESTING - MM API + Web Authentication")
    print("="*80)
    ...
    testers = [BetProphetTester(account) for account in ACCOUNTS]
    ...
    # Step 3: Coordinated Betting for GEC Generation from 3 Events
    ...
    if success:
        print("✅ Parallel exposure testing completed successfully!")
        ...
        print("❌ Parallel exposure testing encountered issues")

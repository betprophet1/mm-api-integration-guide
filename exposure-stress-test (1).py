...
# Odds choices for random selection
# Note: Odds must be >= 100 or <= -100 (not equal to -100)
# So we use 110, 120, 160 to avoid -100 when negated
ODDS_CHOICES = [110, 120, 160]

# Matching rate: configurable probability that each account places opposing bet to itself (self-match)
MATCHING_RATE = 0.5  # Default 50% self-match rate (can be changed via --matching-rate arg)

# Account credentials - 4 accounts for parallel betting
ACCOUNTS = [
    {
        "id": 1,
        "name": "Account 1",
        ...
        "user_uuid": "a9702e96-7a59-43b9-8bae-74d38b5a4566"
    },
    {
        "id": 2,
        "name": "Account 2", 
        ...
        "user_uuid": "4a95fb4b-39bc-43a8-915f-05e0ea380b6b"
    },
    {
        "id": 3,
        "name": "Account 3",
        "email": "thanos_exposure_19300@gmail.com", 
        "password": "123123Aa",
        "access_key": "fd0a93b1215c35c67b692d09cd650f43",
        "secret_key": "c006009cab7bb77f80e3b63c8842dfef",
        "user_uuid": "6e7a16d4-c154-49d6-b764-a16289d27929"
    },
    {
        "id": 4,
        "name": "Account 4", 
        "email": "thanos_exposure_19301@gmail.com",
        "password": "123123Aa", 
        "access_key": "c958baa4a0660946c2dfd877acdec4ab",
        "secret_key": "9882c8ff7f6b97801c81527713d6870e",
        "user_uuid": "3a213635-88e1-4417-ab49-b3db9436c39"
    }
]
...
def generate_performance_report(environment, num_workers, elapsed_time):
    ...
    report_lines = [
        ...
        f"Number of Accounts: {len(performance_data['account_stats'])}",
        "",
        "=" * 80,
        "RESULTS SUMMARY",
        "=" * 80,
...
def stress_test_worker(worker_id, testers, market_data, num_bets):
    """Worker thread - Each account places bet & optionally self-matches with opposing bet"""
    global should_stop, bets_placed_count, bets_cancelled_count, gec_events_count, target_bets, performance_data
    
    num_accounts = len(testers)
    print(f"🔥 Worker {worker_id}: Starting with target {num_bets} bets")
    print(f"   Each account has {MATCHING_RATE*100:.0f}% chance to self-match (place 2 opposing bets)")
    ...
    # Buffer to track wagers for cancellation - one per account
    wagers_buffers = [[] for _ in range(num_accounts)]
    ...
    while not should_stop and placed_by_worker < num_bets:
        ...
        try:
            ...
            # Each account places bet(s) independently
            # MATCHING_RATE determines if account places 2 opposing bets (self-match)
            stake = 5.0
            bets_placed_this_round = 0
            
            # Process each account
            for i in range(num_accounts):
                # Random odds value for this account
                odds_value = random.choice(ODDS_CHOICES)
                
                # Randomly choose first outcome (4 or 5)
                first_outcome = random.choice([4, 5])
                
                # Get appropriate line_id and odds sign
                if first_outcome == 4:
                    line_id_1 = outcome_4[0]['line_id']
                    odds_1 = odds_value
                    line_id_2 = outcome_5[0]['line_id']
                    odds_2 = -odds_value
                else:
                    line_id_1 = outcome_5[0]['line_id']
                    odds_1 = -odds_value
                    line_id_2 = outcome_4[0]['line_id']
                    odds_2 = odds_value
                
                # Place first bet
                bet1 = testers[i].place_bet(line_id_1, odds_1, stake)
                if bet1:
                    wagers_buffers[i].append(bet1)
                    bets_placed_this_round += 1
                else:
                    worker_errors += 1
                
                time.sleep(0.05)  # Brief delay
                
                # MATCHING_RATE chance to place opposing bet (self-match)
                if random.random() < MATCHING_RATE:
                    bet2 = testers[i].place_bet(line_id_2, odds_2, stake)
                    if bet2:
                        wagers_buffers[i].append(bet2)
                        bets_placed_this_round += 1
                        successful_pairs += 1  # This is a self-matching pair
                    else:
                        worker_errors += 1
            ...
            # Update counters
            if bets_placed_this_round > 0:
                placed_by_worker += bets_placed_this_round
                if bets_placed_this_round == num_accounts:
                    successful_pairs += 1
                ...
                with stats_lock:
                    performance_data['bet_timeline'].append({
                        'timestamp': time.time(),
                        'worker_id': worker_id,
                        'bets_placed': bets_placed_this_round
                    })
            else:
                failed_attempts += 1
                if failed_attempts >= max_failed_attempts:
            ...
            for i in range(num_accounts):
                if len(wagers_buffers[i]) >= 20:
                    cancelled = testers[i].batch_cancel_wagers(wagers_buffers[i][:20])
                    cancelled_by_worker += cancelled
                    wagers_buffers[i] = wagers_buffers[i][20:]
            ...
            time.sleep(0.05)  # Very brief delay between bet pairs for faster matching
    ...
    # Cancel remaining wagers in buffers for all accounts
    print(f"🧹 Worker {worker_id}: Cleaning up remaining wagers...")
    for i in range(num_accounts):
        while wagers_buffers[i] and not should_stop:
            cancelled = testers[i].batch_cancel_wagers(wagers_buffers[i][:20])
            cancelled_by_worker += cancelled
            wagers_buffers[i] = wagers_buffers[i][20:]
            time.sleep(0.05)  # Faster cleanup
...
def run_stress_test(num_workers=5, environment='sandbox', event_id=None, matching_rate=0.5):
    """
    Run the exposure stress test with self-matching pattern
    
    Each account independently:
    1. Places 1 bet on a random outcome (4 or 5) with random odds
    2. With matching_rate probability, places opposing bet on opposite outcome with same odds value
    
    Example with matching_rate=0.5:
    - Account 1: Bet 1 on outcome 4 (+160), then 50% chance → Bet 2 on outcome 5 (-160)
    - Account 2: Bet 1 on outcome 5 (-120), then 50% chance → Bet 2 on outcome 4 (+120)
    - etc.
    ...
    :param matching_rate: Probability each account self-matches (default 0.5 = 50%)
    """
    global should_stop, bets_placed_count, gec_events_count, target_bets, target_gec_events, performance_data, MATCHING_RATE
    
    # Update global matching rate
    ...
    print(f"�📊 Matching Rate: {matching_rate*100:.0f}% (bet2 uses same odds as bet1)")
    if event_id:
        print(f"🎯 Using specific event ID: {event_id}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Exposure stress test: self-matching pattern - each account places opposing bets')
    ...
    parser.add_argument('--matching-rate', type=float, default=0.5,
                       help='Probability that each account places opposing bet to itself (0.0-1.0, default: 0.5 = 50%%)')
    ...
    try:
        run_stress_test(num_workers=args.workers, environment=args.env, event_id=args.event_id, matching_rate=args.matching_rate)
    except Exception as e:

#!/usr/bin/env python3
"""
Wallet Retry Logic Test - Modified to exclude MM1 (deduce-enabled account)
...
MODIFICATION: Excludes MM1 account to avoid using deduce-enabled account
"""
...
def test_submit_wager_with_retry_delays(event_id=None, num_tests=10):
    ...
    Uses 9 MM accounts (MM2-MM10) to test concurrent retry behavior - EXCLUDES MM1
    """
    print(f"\n{Colors.BOLD}{Colors.MAGENTA}{'='*70}{Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.MAGENTA}Test: Submit Wager Retry Delays (45 WORKERS - AGGRESSIVE){Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.MAGENTA}Testing wallet retry behavior với different delays{Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.RED}EXCLUDING MM1 (deduce-enabled account){Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.MAGENTA}{'='*70}{Colors.RESET}\n")
    
    framework = DeduceTestFramework(environment=config.ENVIRONMENT)
    
    # Setup 45 MM workers using accounts MM2-MM10 (excluding MM1)
    print(f"{Colors.CYAN}📦 Setting up 45 MM workers (using 9 accounts x 5 workers each, EXCLUDING MM1)...{Colors.RESET}\n")
    
    # First, login to 9 actual MM accounts (MM2 to MM10, skip MM1)
    base_mm_accounts = []
    for i in range(2, 11):  # Changed from range(1, 11) to range(2, 11) to exclude MM1
        mm_name = f'mm{i}'
        try:
    ...
    # Create 45 virtual workers by reusing the 9 accounts (5 workers per account)
    mm_accounts = []
    for worker_id in range(45):  # Changed from 50 to 45
        account_index = worker_id % len(base_mm_accounts)
        mm_accounts.append(base_mm_accounts[account_index])
    
    print(f"\n{Colors.GREEN}✅ Created {len(mm_accounts)} workers from {len(base_mm_accounts)} accounts (EXCLUDING MM1){Colors.RESET}")
    print(f"{Colors.YELLOW}⚠️  WARNING: This will stress test with {len(mm_accounts)} concurrent requests!{Colors.RESET}\n")
    
    # Get market using MM2 instead of MM1
    print(f"{Colors.CYAN}🔍 Finding market...{Colors.RESET}")
    if event_id:
        from test_deduce_race_conditions import get_market_for_event
        market_info = get_market_for_event(framework, 'mm2', event_id)  # Changed from 'mm1' to 'mm2'
    else:
        market_info = framework.get_available_market('mm2')  # Changed from 'mm1' to 'mm2'
...
def test_cancel_wager_retry_race_condition(event_id=None, num_tests=10):
    """
    Test Case: Cancel Wager Retry Race Condition - Using MM2 instead of MM1
    ...
    print(f"{Colors.BOLD}{Colors.RED}USING MM2 (excluding deduce-enabled MM1){Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.MAGENTA}{'='*70}{Colors.RESET}\n")
    
    framework = DeduceTestFramework(environment=config.ENVIRONMENT)
    
    # Setup - using MM2 instead of MM1
    print(f"{Colors.CYAN}📦 Setting up account (MM2)...{Colors.RESET}\n")
    mm2_creds = config.get_account_credentials(2, config.ENVIRONMENT)  # Changed from 1 to 2
    framework.login_account('mm2', mm2_creds, account_type='mm')  # Changed from 'mm1' to 'mm2'
    
    initial_balance = framework.get_balance('mm2').get('balance', 0)  # Changed from 'mm1' to 'mm2'
    print(f"{Colors.GREEN}✅ Initial balance (MM2): ${initial_balance:,.2f}{Colors.RESET}\n")
    ...
    for delay in cancel_delays:
        ...
        balance_before = framework.get_balance('mm2').get('balance', 0)  # Changed from 'mm1' to 'mm2'
        
        # Submit wager
        odds = 150
        stake = 2.0
        
        print(f"{Colors.YELLOW}📤 Submitting wager (MM2)...{Colors.RESET}")
        submit_result = framework.place_wager('mm2', line_id, odds, stake)  # Changed from 'mm1' to 'mm2'
        ...
        cancel_success1 = framework.cancel_wager('mm2', external_id, wager_id)  # Changed from 'mm1' to 'mm2'
        ...
        cancel_success2 = framework.cancel_wager('mm2', external_id, wager_id)  # Changed from 'mm1' to 'mm2'
        ...
        # Check balance
        balance_after = framework.get_balance('mm2').get('balance', 0)  # Changed from 'mm1' to 'mm2'
        balance_change = balance_before - balance_after
...
def test_concurrent_submit_cancel_with_retries(event_id=None, duration=30):
    """
    Test Case: Concurrent Submit + Cancel với Retries - Using MM2 instead of MM1
    ...
    def submit_worker():
        ...
        while time.time() - start_time < duration:
            try:
                ...
                # First attempt
                result = framework.place_wager('mm2', line_id, odds, stake)  # Changed from 'mm1' to 'mm2'
                ...
                if result.get('success'):
                    ...
                    # Retry
                    retry_result = framework.place_wager('mm2', line_id, odds, stake)  # Changed from 'mm1' to 'mm2'
        ...
        while time.time() - start_time < duration:
            ...
            if wager_info:
                try:
                    ...
                    cancel_success = framework.cancel_wager('mm2', external_id, wager_id)  # Changed from 'mm1' to 'mm2'
                    ...
                    if cancel_success:
                        ...
                        # Retry
                        retry_success = framework.cancel_wager('mm2', external_id, wager_id)  # Changed from 'mm1' to 'mm2'
                        if retry_success:
                            with lock:
                                stats['cancelled'] += 1
                        else:
    ...
    # Final balance
    final_balance = framework.get_balance('mm2').get('balance', 0)  # Changed from 'mm1' to 'mm2'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Test wallet retry logic and deadlock scenarios (excluding MM1)')
    ...
    print(f"{Colors.BOLD}{Colors.RED}NOTICE: This script excludes MM1 (deduce-enabled account) from all tests{Colors.RESET}\n")
